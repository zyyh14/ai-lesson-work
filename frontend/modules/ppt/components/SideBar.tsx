...
import { ArrowLeft, Plus, FileUp, MoreVertical, Trash2, Edit2, FileText, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      >
        ...
        <div className="px-4 pt-4 min-w-[280px]">
          <button
            onClick={() => {
              const url = getEnvUrl('VITE_TEACHER_DASHBOARD_URL', 'http://localhost:3001/dashboard');
              window.location.href = url;
            }}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 transition-colors border border-slate-700"
            type="button"
          >
            <ArrowLeft size={16} />
            返回工作台
