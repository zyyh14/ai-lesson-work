...
function extractJsonFromContent(raw: string): any {
  try {
    ...
    let jsonText = afterThink.slice(firstBrace, lastBrace + 1);
    ...
    // 2. 修复常见的转义问题
    // 修复 LaTeX 数学公式中的反斜杠（在字符串值中）
    // 将字符串中的 \( \) \[ \] \{ \} 等替换为普通括号
    jsonText = jsonText.replace(/"([^"]*?)"/g, (match, content) => {
      // 在字符串内容中修复常见的转义问题
      let fixed = content
        .replace(/\\[()\[\]{}]/g, (m: string) => m.charAt(1))  // \( -> (
        .replace(/\\\\/g, '\\')  // \\ -> \
        .replace(/\\n/g, ' ')    // \n -> 空格
        .replace(/\\t/g, ' ')    // \t -> 空格
        .replace(/\\r/g, '')     // \r -> 空
    ...
    try {
      ...
      console.warn("首次 JSON 解析失败，尝试修复常见问题...");
      console.warn("错误信息:", firstError);
      
      // 尝试修复：移除所有控制字符
      const fixedControl = jsonText.replace(/[\x00-\x1F\x7F-\x9F]/g, '');
      try {
        return JSON.parse(fixedControl);
      } catch (secondError) {
        // 尝试修复：更激进的清理 - 移除所有非法的反斜杠
        const fixedBackslash = fixedControl.replace(/\\(?!["\\/bfnrtu])/g, '');
        try {
          return JSON.parse(fixedBackslash);
        } catch (thirdError) {
          // 最后尝试：将所有单反斜杠替换为双反斜杠（除了已知的转义序列）
          const fixedEscape = fixedControl.replace(/\\(?=[^"\\/bfnrtu])/g, '\\\\');
          try {
            return JSON.parse(fixedEscape);
          } catch (fourthError) {
            console.error("所有修复尝试都失败了");
            console.error("最后尝试的文本:", fixedEscape.substring(0, 500));
            throw firstError;
          }
        }
      }
