...
import { Info, Bell, ArrowLeft, FileSpreadsheet, Image as ImageIcon } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 md:pl-64">
      ...
      <header className="bg-white h-16 border-b border-slate-200 sticky top-0 z-10 px-8 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex bg-slate-100 p-1 rounded-lg ml-2">
            <button
              type="button"
              onClick={() => setActiveTab('grades')}
              className={`px-3 py-1.5 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'grades' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              <FileSpreadsheet className="w-4 h-4" /> 成绩单
            </button>
            <button
              type="button"
              onClick={() => setActiveTab('paper')}
              className={`px-3 py-1.5 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'paper' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
