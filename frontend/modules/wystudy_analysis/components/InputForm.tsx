...
export const InputForm: React.FC<InputFormProps> = ({ onSubmit, loadingState, activeTab, onTabChange }) => {
  const currentTab = activeTab || 'grades';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const dataToSubmit: ClassInput = currentTab === 'grades' 
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
      <div className="bg-slate-900 px-6 py-4 flex justify-between items-center flex-wrap gap-4">
        ...
        <div className="flex bg-slate-800 p-1 rounded-lg">
          <button 
             ...
             className={`px-4 py-2 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${currentTab === 'grades' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}
          >
          ...
          <button 
             ...
             className={`px-4 py-2 rounded-md text-sm font-bold transition-all flex items-center gap-2 ${currentTab === 'paper' ? 'bg-pink-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}
          >
      ...
      <form onSubmit={handleSubmit} className="p-6 md:p-8">
        ...
        {/* Tab Content: Grades */}
        <div className={currentTab === 'grades' ? 'block animate-fade-in' : 'hidden'}>
        ...
        {/* Tab Content: Paper */}
        <div className={currentTab === 'paper' ? 'block animate-fade-in' : 'hidden'}>
        ...
        <div className="flex justify-center mt-8 pt-6 border-t border-slate-100">
           <button
            type="submit"
            disabled={isGenerating || processingImage || (currentTab === 'grades' && !formData.gradeSheetText && !formData.gradeSheetImage) || (currentTab === 'paper' && !formData.paperImage)}
            className={`
              ...
              ${isGenerating || processingImage
                ...
                : (currentTab === 'grades' && !formData.gradeSheetText && !formData.gradeSheetImage) || (currentTab === 'paper' && !formData.paperImage)
          ...
          >
            {isGenerating || processingImage ? (
              ...
              <>
                ...
                {currentTab === 'grades' ? '生成成绩分析报告' : '生成试卷诊断报告'}
              </>
            )}
          </button>
        </div>
