...
"""
    ...
    def _is_zhipu_key_configured(self) -> bool:
        key = (settings.ZHIPU_API_KEY or "").strip()
        if not key:
            return False
        if key == "placeholder_zhipu_key":
            return False
    ...
    async def generate_exercises(self, knowledge_point: str) -> Dict[str, Any]:
        ...
        """
        try:
            if not self._is_zhipu_key_configured():
                return {
                    "knowledge_point": knowledge_point,
                    "exercises": [],
                    "error": "ZHIPU_API_KEY 未配置或仍为 placeholder，请在 python_service/.env 中设置真实的智谱 API Key",
                    "status": "error"
                }
            # 调用LLM生成练习题
